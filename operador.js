...
async function carregarAtividade() {
    ...
    const loadingPopup = L.popup({ closeButton: false, autoClose: false }).setLatLng(map.getCenter()).setContent(loadingMessage).openOn(map);
    ...
    try {
        ...
        if(allFeatures.length === 0) {
            alert("As quadras desta atividade não foram encontradas. Verifique se os arquivos de dados estão corretos.");
            return;
        }
